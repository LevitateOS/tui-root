# install disk-plan (recpart-backed)

Canonical install disk-planning UI owner.
...
- UI app (`tui/apps/install/disk-plan`) owns flow, page transitions, and evidence presentation.
...
```bash
just tui-install-disk-plan -- --disk /dev/sdX --mode ab
```
