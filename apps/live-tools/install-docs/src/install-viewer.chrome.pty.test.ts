...
const XTASK_BIN = ensureXtaskBinary();

function runPtyCapture(columns: number, rows: number): string {
  return runDocsInspectCapture(columns, rows, "installation");
}

function runPtyCaptureSlug(columns: number, rows: number, slug: string): string {
  return runDocsInspectCapture(columns, rows, slug);
}

function runDocsInspectCapture(columns: number, rows: number, slug: string): string {
  const outDir = mkdtempSync(join(tmpdir(), "docs-tui-chrome-pty-"));
  ...
  try {
    if (result.status !== 0) {
      throw new Error(
        [
          `docs inspect capture failed with exit ${result.status ?? "unknown"}.`,
          result.stdout ?? "",
          result.stderr ?? "",
        ]
          .join("\n")
    ...
    const runDir = readdirSync(outDir, { withFileTypes: true })
      .filter((entry) => entry.isDirectory() && entry.name.startsWith("run-"))
      .map((entry) => entry.name)
      .sort()
      .at(-1);
    if (!runDir) {
      throw new Error(`missing xtask transcript run directory under '${outDir}'`);
    }
    return readFileSync(join(outDir, runDir, `001-${slug}.transcript`), "utf8");
  } finally {
    rmSync(outDir, { recursive: true, force: true });
  }
}

function ensureXtaskBinary(): string {
  const binary = join(REPO_ROOT, "target/debug/levitate-xtask");
  const result = spawnSync("cargo", ["build", "-p", "levitate-xtask"], {
    cwd: REPO_ROOT,
    env: { ...process.env, TERM: "xterm-256color" },
    encoding: "utf8",
  });
  if (result.status !== 0 || !existsSync(binary)) {
    throw new Error(
      [
        `failed to build levitate-xtask (exit ${result.status ?? "unknown"}).`,
        result.stdout ?? "",
        result.stderr ?? "",
      ]
        .join("\n")
...
function segmentsForInspection(transcript: string): string[][] {
  const segments = splitSegments(transcript).filter((segment) => segment.length > 0);
  if (segments.length === 0) {
    throw new Error("missing captured terminal segments");
  }
...
describe("install viewer right-panel chrome", () => {
  it("draws seam semantics with box-drawing glyphs in segments 0 and 1", () => {
    for (const columns of [96, 120]) {
      const segments = segmentsForInspection(runPtyCapture(columns, 32));

      for (const [segmentIndex, lines] of segments.entries()) {
        const inspection = inspectSegment(lines, segmentIndex, "right", TITLE_PATTERN);
        if (!inspection.ok) {
          throw new Error(
...
describe("install viewer sidebar chrome", () => {
  it("renders Installation Docs separator as a structural seam in segments 0 and 1", () => {
    for (const columns of [96, 120]) {
      ...
      for (const [segmentIndex, lines] of segments.entries()) {
        const inspection = inspectSegment(lines, segmentIndex, "left", /\bInstallation Docs\b/);
        if (!inspection.ok) {
          throw new Error(
            `columns=${columns} segment=${segmentIndex} failed: ${inspection.reason ?? "unknown"}; span=[${inspection.span?.left ?? -1},${inspection.span?.right ?? -1}] title=${inspection.titleIndex ?? -1} seam=${inspection.seamIndex ?? -1} bottom=${inspection.bottomIndex ?? -1} top='${inspection.topSegment ?? ""}' seam='${inspection.seamSegment ?? ""}' bottom='${inspection.bottomSegment ?? ""}'`,
          );
...
describe("helpers code-fence geometry", () => {
  it("keeps side borders aligned across wrapped and blank rows", () => {
    for (const columns of [96, 120]) {
      const segments = segmentsForInspection(runPtyCaptureSlug(columns, 36, "helpers-commands"));
