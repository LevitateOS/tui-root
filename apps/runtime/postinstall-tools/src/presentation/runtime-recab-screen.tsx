...
export function RuntimeRecabScreen({ recabBin }: RuntimeRecabScreenProps) {
  ...
  const footer = useMemo(() => {
    const segments = [
      scopeStatusSegment("runtime-postinstall-tools"),
  ...
  const body = (
    <Box flexDirection="column">
      ...
      <UiText>Runtime actions over the existing recab backend.</UiText>
      <UiText> </UiText>
      ...
      <UiText intent="dimText">
        Command: {`${recabBin} ${activeAction.command.args.join(" ")}`}
      </UiText>
      <UiText> </UiText>
